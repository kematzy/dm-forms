  ...
  module Form
    module Builder
      ...
      class Base
        ...
        def options col, text_meth, value_meth, sel, b = nil 
          ([b] + col.map do |item|
            ...
            text = text.escape_html
            value = value.escape_html
      ...
      class Form < Base
        ...
        def update_bound_controls method, attrs, type 
          attrs.merge!(:id => "#{@name}-#{method}") unless attrs[:id]
          super
        end
        ...
        def radio_group_item method, attrs 
          unless attrs[:id]
            attrs.merge!(:id => "#{@name}-#{method}-#{attrs[:value]}")
          end
